...
#line 107 "..\\bsp_lib\\nrf24l01.c"
	...
	SPI_I2S_DeInit(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)));
...
static unsigned char spi_read_reg(unsigned char reg){
	...
	SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), reg);
	wait();
	SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), 0x00);
...
static void spi_tx_byte(unsigned char reg, unsigned char value){
	...
	SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), reg);
	wait();
	SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), value);
	wait();
...
static void SPI_Write_Buf(unsigned char reg, unsigned char *pBuf, unsigned char bytes){
	...
	wait();
	while(bytes--){
		SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x3000)), *pBuf++);
		wait();
	}
...
void NRF24L01_TX_Mode(u8 *addr){
	...
	spi_tx_byte(0xE1,0x00);
	SPI_Write_Buf(0x20 + 0x10, addr, 5);    
	SPI_Write_Buf(0x20 + 0x0A, addr, 5); 
	spi_tx_byte(0x20 + 0x01, 0x01);      
	spi_tx_byte(0x20 + 0x02, 0x01);  
	spi_tx_byte(0x20 + 0x04, 0xFF); 
	spi_tx_byte(0x20 + 0x05, 40);        
	spi_tx_byte(0x20 + 0x06, 0x07);   
	spi_tx_byte(0x20 + 0x11, 32); 
	spi_tx_byte(0x20 + 0x00, 0x0e);
	
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0010));

	sta = spi_read_reg(0x07);
	spi_tx_byte(0x20+0x07, sta);	
	Delay_Ms(10);
	return;
}
...
int NRF24L01_Send(unsigned char *data){
	...
	spi_tx_byte(0xE1,0x00);
	GPIO_ResetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)), ((uint16_t)0x0010));
	SPI_Write_Buf(0xA0, data, 32);     
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x0800)), ((uint16_t)0x0010));
	
	GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0010));
	while(GPIO_ReadInputDataBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0020)) == Bit_SET){
		cnt++;
		Delay_Ms(2);
		if(cnt>100){
			Delay_Ms(2000);
	...
	while(GPIO_ReadInputDataBit(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x1000)), ((uint16_t)0x0020)) == Bit_RESET){
		sta = spi_read_reg(0x07);
		spi_tx_byte(0x20+0x07, sta);
		Delay_Ms(10);
		cnt++;
