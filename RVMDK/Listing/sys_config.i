...
void RCC_Configuration(void) 
{
	...
	RCC_APB2PeriphClockCmd(((uint32_t)0x00000004) | ((uint32_t)0x00000008) | ((uint32_t)0x00000010) |
												 ((uint32_t)0x00001000)  | ((uint32_t)0x00000001), ENABLE);
...
void save_addr(u8 *addr){
	...
	while(count < 5){
		FLASH_ProgramHalfWord((0x8008000 +count*2),addr[count]);  
		count++;
	}
	FLASH_Lock();
...
void ad_init(void){
	...
	ADC_RegularChannelConfig(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x10000) + 0x2400)), ((uint8_t)0x08) , 1, ((uint8_t)0x07));        
